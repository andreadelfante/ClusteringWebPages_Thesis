...
\subsection{Parametri degli algoritmi di Clustering}
...
\\
In questo caso, le configurazioni sono state selezionate in base al più alto numero di valori più alti delle metriche, e sono state raccolte in Tabella \ref{bestleftnormallineillinois} e confrontate con LINE. Si evince come LINE non riesca ad estrarre informazioni utili al Clustering dalla struttura ad hyperlink del sito: la bontà dei risultati, infatti, risulta piuttosto bassa.
\\\\
Considerando solo le informazioni estraibili dalla struttura ad hyperlink del sito Illinois, l'algoritmo che le estrae in maniera efficiente è \textbf{normalSkipgram} avente configurazione \textbf{[100K, 2, 20]} e \textbf{senza liste di costrizione}, abbinato a \textbf{KMeans}. In questa configurazione, le liste di costrizione non hanno migliorato la qualità dei risultati: normalSkipgram è riuscito ad estrarre informazioni utili pur avendo un numero di frasi relativamente piccolo (100.000) avente lunghezza di 20 URL. La finestra di contesto di piccola dimensione (2) ha permesso a normalSkipgram di escludere tutte quelle informazioni che potevano compromettere la bontà dell'operazione di Clustering.
...
Come nella configurazione precedente, anche trattando le pagine del sito Illinois come semplici documenti si evince come si riescono ad estrapolare dal testo più informazioni significative utilizzando \textbf{Doc2Vec} con il dataset \textbf{senza costrizioni}, che vengono utilizzate in maniera più opportuna da  \textbf{HDBScan} (Tabella \ref{doc2vectfidfillinois}).
...
In Tabella \ref{combillinois} sono stati raccolti i risultati del Clustering utilizzando sia l'informazione codificata nella struttura ad hyperlink sia quella estrapolata dal testo. Ai fini della concatenazione, sono stati considerati per leftSkipgram e normalSkipgram solo le configurazioni migliori, opportunamente analizzate e scelte in precedenza. Si evince come \textbf{HDBScan}, sfruttando l'informazioni estratta da \textbf{leftSkipgram} avente la configurazione ottimale [1M, 2, 20] e combinato con i vettori prodotti da \textbf{Tf-Idf}, riesca ad assegnare allo stesso Cluster pagine appartenenti alla stessa classe. Anche in questo caso, le liste non hanno migliorato la qualità dei Cluster.
...
Nelle Tabelle \ref{leftskipgramrwlckmeansoxford}, \ref{leftskipgramrwlchdbscanoxford}, \ref{normalskipgramrwlckmeansoxford}, \ref{normalskipgramrwlchdbscanoxford}, \ref{leftskipgramrwnckmeansoxford}, \ref{leftskipgramrwnchdbscanoxford}, \ref{normalskipgramrwnckmeansoxford}, \ref{normalskipgramrwnchdbscanoxford}, come nel caso precedente, sono stati riportati i risultati di leftSkipgram e normalSkipgram per tutte le combinazioni, sia per KMeans che per HDBScan. La Tabella \ref{normalskipgramrwnchdbscanoxford} mostra che, in realtà, i risultati migliori sono concentrati nelle configurazioni [1M, 2, 15] e [1M, 3, 20]. Confrontandole, si nota come la seconda, a parità di valori, mostra un punteggio più alto nella metrica Completezza (0.02), riuscendo ad assegnare, in maniera più opportuna, elementi della stessa classe allo stesso raggruppamento rispetto alla prima configurazione.
\\
Come in precedenza, sono state scelte le migliori configurazioni di numero di URL, dimensione della finestra e lunghezza delle frasi, e sono state confrontate con LINE. In Tabella \ref{bestleftnormallineoxford} si nota come i punteggi delle metriche sono concentrate su \textbf{leftSkipgram senza costrizioni}, con una configurazione di \textbf{[1M, 5, 20]}. In questo sito, quindi, analizzare solo il contesto sinistro con una dimensione standard della finestra di contesto (5) ha permesso di estrarre informazioni più pertinenti dalla struttura di Oxford, che sono state sfruttate a pieno da \textbf{KMeans}. Anche in questo caso, le Liste Web non hanno contribuito a migliorare la qualità dei Cluster.
\\
...
Nelle Tabelle \ref{leftskipgramrwlckmeanspriceton}, \ref{leftskipgramrwlchdbscanpriceton}, \ref{normalskipgramrwlckmeanspriceton}, \ref{normalskipgramrwlchdbscanpriceton}, \ref{leftskipgramrwnckmeanspriceton}, \ref{leftskipgramrwnchdbscanpriceton}, \ref{normalskipgramrwnckmeanspriceton}, \ref{normalskipgramrwnchdbscanpriceton} sono stati raccolti i valori delle metriche, sia per leftSkipgram che per normalSkipgram, come nei casi precedenti. Come in precedenza, anche in questo caso sono state scelte le configurazioni aventi maggior numero di valori delle metriche più alti. Nel caso della Tabella
\ref{leftskipgramrwlckmeanspriceton} è stata scelta come configurazione migliore per KMeans con leftSkipgram con liste di costrizione [100K, 7, 20] poiché, pur non avendo i più alti valori in Completezza ed Omogeneità, ha il maggior punteggio in V-Measure: di conseguenza, presenta un certo equilibrio tra le metriche precedentemente citate.
\\
Le migliori configurazioni sono state scelte e raccolte nella Tabella \ref{bestleftnormallinepriceton} dove, confrontandole con LINE di primo e secondo ordine di prossimità, \textbf{normalSkipgram con liste di costrizione}, con una configurazione di \textbf{[1M, 5, 10]} affiancato a \textbf{KMeans}, ottiene punteggi più alti. Considerando sia contesto destro che quello sinistro, con una dimensione della finestra pari a 5 (quindi standard), normalSkipgram ha estratto informazioni utili pur considerando un numero discreto relazioni indirette tra le pagine di Priceton. In questo caso, le Liste Web hanno migliorato la qualità dei Cluster creati.
\\
E' interessante notare come LINE con secondo ordine di prossimità, applicato sul dataset di Priceton con liste di costrizione, permette di estrarre informazioni utili per aumentare la forma dei raggruppamenti: infatti questo algoritmo permette di ottenere, abbinato ad HDBScan, un punteggio di Silhouette pari a 0.21, un valore che si distacca in maniera decisiva da quelli degli altri algoritmi messi a confronto. Non solo, sempre utilizzando HDBScan e considerando il dataset senza liste di costrizione, LINE con primo ordine di prossimità permette di estrarre informazioni per aumentare il valore di Completezza, riuscendo a raggiungere lo 0.62, permettendo dunque di raggruppare tutti gli elementi di una classe in uno stesso Cluster.
...
Considerando le pagine di Priceton come semplici documenti testuali, da questo caso della sperimentazione, i cui risultati sono stati inseriti nella Tabella \ref{doc2vectfidfpriceton}, si è scoperto che la matrice di \textbf{Tf-idf} ha permesso di aumentare le performance di \textbf{KMeans}. Le Liste Web non sono servite per aumentare la qualità dei Cluster generati, probabilmente perché venivano escluse, dal Crawler che sfrutta le Liste Web, pagine aventi un contenuto testuale utile per migliorare il processo di raggruppamento delle stesse.
...
I risultati prodotti dalla combinazione dei vettori prodotti dai diversi algoritmi, raggruppati nella Tabella \ref{combpriceton}, hanno messo in evidenza che, considerando le pagine indicizzate dal Crawler tradizionale, \textbf{KMeans}, usando le informazioni estratte dalla combinazione \textbf{normalSkipgram}, con combinazione [1M, 7, 10] e \textbf{Tf-Idf}, ha prodotto dei Cluster con un certo livello di bontà. Le Liste Web non hanno migliorato la qualità del processo di Clustering.
...
Nelle Tabelle \ref{leftskipgramrwlckmeansstanford}, \ref{leftskipgramrwlchdbscanstanford}, \ref{normalskipgramrwlckmeansstanford}, \ref{normalskipgramrwlchdbscanstanford}, \ref{leftskipgramrwnckmeansstanford}, \ref{leftskipgramrwnchdbscanstanford}, \ref{normalskipgramrwnckmeansstanford}, \ref{normalskipgramrwnchdbscanstanford} sono stati raccolti i valori delle metriche, sia per leftSkipgram che per normalSkipgram. Come effettuato in precedenza, sono state scelte le configurazioni aventi il maggior numero di valori delle metriche più alti. Si può notare come le configurazioni che risultano migliori, in realtà, non hanno permesso agli algoritmi di Clustering di raggiungere elevate performance come nei precedenti casi. Evidentemente, la struttura ad hyperlink del sito Web non ha permesso di effettuare un processo di Clustering avente una buona qualità. Si può dedurre che Stanford è un sito Web non altamente strutturato.
\\
Inoltre, in Tabella \ref{normalskipgramrwnchdbscanstanford}, sono due le configurazioni aventi il maggior numero di valori più alti delle metriche: [500K, 2, 20] e [500K, 3, 20]. Si è scelta come configurazione ottimale [500K, 3, 20], poiché, pur avendo gli stessi valori delle stesse metriche, differisce in Silhouette, con un punteggio più alto dello 0.01. A confronto, la configurazione scelta ha permesso di avere dei raggruppamenti aventi una forma leggermente migliore, pur avendo entrambe prodotto Cluster sovrapposti: entrambi i valori, infatti, sono vicini allo 0.
\\
In Tabella \ref{bestleftnormallinestanford} sono state raccolte tutte le migliori configurazioni di leftSkipgram e normalSkipgram, e confrontate con entrambi gli ordini di prossimità di LINE. L'unico aspetto interessante è il valore della Silhouette prodotto da LINE di secondo ordine su un dataset con liste di costrizione, affiancato ad HDBScan pari a 0.01, un valore decisamente più alto ottenuti tra gli algoritmi di apprendimento utilizzati su questo sito. I Cluster prodotti da HDBScan, utilizzando solo l'informazione strutturale ottenuta dalle relazioni indirette tra pagine considerate fratelli (quindi che condividono pagine immediatamente vicine), sono più definiti. I risultati più alti sono ottenuti da \textbf{normalSkipgram con liste di costrizione}, affiancato a \textbf{KMeans}, avente configurazione \textbf{[1M, 2, 20]}. Analizzare, quindi, pagine collegate tra loro da molti elementi, non permette di aumentare le performance del Clustering. Le Liste Web hanno permesso di filtrare in maniera consistente le pagine esplorate in quanto il sito includeva la maggior parte degli hyperlink in sezioni nascoste, prevalentemente in menu dropdown (a scomparsa).
...
In generale, si nota immediatamente che, utilizzando questi siti Web, il miglior algoritmo di Clustering che riesce a sfruttare in modo migliore le informazioni estratte è KMeans. Una analisi più approfondita ci dice che la versione tradizionale di Skip-Gram estrae informazioni più utili rispetto a quella modificata: l'esclusione dall'analisi della struttura del sito la parte destra del contesto filtra informazioni che potrebbero migliorare il processo di Clustering delle pagine del sito Web. Inoltre più è alto il numero di frasi generate e di lunghezza massima di esplorazione del Crawler, maggiore è la quantità di informazioni, estratte dalla struttura ad hyperlink del sito, utili per migliorare la bontà dei Cluster generati. Non solo, anche considerare il minor numero di relazioni indirette tra due pagine migliora il Clustering.
\\\\
L'uso delle liste non aumenta in maniera significativa le performance degli algoritmi di Clustering. Questo può essere l'effetto di due cause: o il Crawler che sfrutta le Liste Web non funziona, oppure che i siti Web in analisi sono altamente strutturati. Effettivamente i siti sono altamente strutturati, poiché se così non fosse l'esperto che ha creato la ground truth non avrebbe potuto effettuare l'etichettatura delle singole pagine, ovvero non avrebbe potuto raggruppare manualmente le pagine.
\\\\
In conclusione, la sperimentazione mostra come i migliori risultati sono ottenuti combinando l'informazione testuale con quella estratta dalla struttura ad hyperlink del sito. Questo è più evidente per Illinois (Tabella \ref{finalresultillinois}) ed Oxford (Tabella \ref{finalresultoxford}), dove la struttura del sito e il testo delle pagine codificano informazioni complementari utili per il Clustering. Per Priceton (Tabella \ref{finalresultpriceton}) l'informazione testuale è soddisfacente per realizzare Cluster di buona qualità; mentre per Stanford (Tabella \ref{finalresultstanford}) l'informazione codificata nella struttura del sito è sufficiente: si può supporre che il sito contenga pochi termini significativi oppure poco testo contenente informazioni utili.