...
\\
Dai testi è possibile estrarre conoscenza utile. La branca dell'Informatica che si occupa di realizzare questo processo è il Text Mining.% Il ramo del Data Mining che si occupa di realizzare questo processo è il Text Mining.
\\\\
...
\end{figure}
\\
Supponiamo di trovarci al passo $t$, dove si trova il primo caso di apprendimento (quick, the). L'obiettivo è quello di predire \textit{the} da \textit{quick}, applicando la formula \ref{skipgrameq} e massimizzando la probabilità media logaritmica per apprendere il vettore ottimale di \textit{the}. Così facendo le parole simili avranno vettori simili. Ad ogni passo vengono aggiornati i vettori delle parole precedentemente apprese.
\\
Questo processo di apprendimento viene ripetuto sull'intera collezione di dati. %[...] vedi se riesci a trovare altro.
...
\subsection{LINE}
\label{line}
LINE è un nuovo modello di network embedding, capace di apprendere rappresentazioni vettoriali di vertici in una rete o grafo. Un grafo è una coppia ordinata $G = (V, E)$ di insiemi, con $V$ insieme dei nodi ed $E$ insieme degli archi.
\\
Questo modello di apprendimento lavora bene soprattutto con grafi orientati, pesati e non-pesati. Viene richiesto come input un file contenente gli archi che costituiscono il grafo da apprendere. L'obiettivo è quello di rappresentare ogni vertice $v \in V$ in un vettore $R^d$ applicando, quindi, una funzione $f_G : V \to R^d$, dove $d << |V|$ \cite{tang2015line}. Viene prodotto, infine, uno spazio vettoriale formato da rappresentazioni vettoriali dei singoli nodi del grafo appreso. Ogni vettore $R^d$ preserva la prossimità di primo e secondo ordine tra i vertici.
...
\begin{itemize}
	...
	\item \textbf{Distanza tra centroidi}. Questa, invece, è la distanza tra i due cluster prendendo in considerazione i centroidi degli stessi.
	\begin{equation}
...
\begin{figure}[h]
	\centering
	\includegraphics[width = 100mm]{dbscan}
