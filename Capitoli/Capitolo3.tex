...
\begin{figure}[h]
	\centering
	\includegraphics[width = 75mm]{WebCrawlerArchitecture}
...
Una volta effettuato il processo di Crawling ed estratti gli URL, si è proceduto alla normalizzazione degli stessi. Questo è un processo in cui gli URL vengono modificati e standardizzati in maniera consistente. Il suo obiettivo è poter determinare se due URL, che sono sintatticamente differenti, possono essere equivalenti.
\\
...
\begin{figure}[h]
	\centering
	\includegraphics[width = 65mm]{randomwalkmono}
...
\begin{figure}[h]
	\centering
	\includegraphics[width = 65mm]{randomwalkbi}
...
\begin{figure}[h]
	\centering
	\includegraphics[width = 65mm]{randomwalktri}
...
I Random Walks prodotti sono stati utilizzati come frasi, le cui parole sono i codici univoci degli URL. L'utilizzo di tali codici ha permesso di ridurre lo spazio in memoria e i tempi di elaborazione.
\\
Le frasi generate sono state sfruttate da Word2Vec allo scopo di apprendere la struttura del grafo del sito Web.
...
\\
Le frasi prodotte dal Random Walker sono state utilizzate sia per Skip-Gram tradizionale che per le sue versioni modificate.
